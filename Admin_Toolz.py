...
class AdminToolsMod(loader.Module):
    ...
    strings = {'name': 'Admin Tools',
               ...
               'wait': '<b>Ждём...</b>',
               'pic_so_small': '<b>Слишком маленькая картинка, попробуй другую.</b>',
               'pic_changed': '<b>Картинка чата сменена.</b>',
               'promote_none': '<b>Некого назначать.</b>',
               'who': '<b>Кто это?</b>',
               'not_admin': '<b>Я здесь не админ.</b>',
               'promoted': '<b>{} выданы права администратора .\nПрефикс: {}</b>',
               'wtf_is_it': '<b>Ну что это блять?</b>',
               'this_isn`t_a_chat': '<b>Это не чат!</b>',
               'demote_none': '<b>Некого лишать прав.</b>',
               'demoted': '<b>{} права администратора сняты.</b>',
               'pinning': '<b>Закрепляю...</b>',
               'pin_none': '<b>Реплаем на смс, что б закрепить.</b>',
               'unpinning': '<b>Отдираю...</b>',
               'unpin_none': '<b>Нечего закреплять.</b>',
               'no_rights': '<b>У меня не достаточно прав для этого.</b>',
               'pinned': '<b>Прилепил!</b>',
               'unpinned': '<b>Отодрал!</b>',
               'can`t_kick': '<b>Не могу выпнуть.</b>',
               'kicking': '<b>Выпинываю...</b>',
               'kick_none': '<b>Некого выгнать.</b>',
               'kicked': '<b>{} был вышвырнут как наглый кот.</b>',
               'kicked_for_reason': '<b>{} вышвернут.\nпотому что: {}.</b>',
               'banning': '<b>Бетонирую...</b>',
               'banned': '<b>{} забанен в чате.</b>',
               'banned_for_reason': '<b>{} Был забанен.\nпотому что: {}</b>',
               'ban_none': '<b>Нет кого банить.</b>',
               'unban_none': '<b>Нет кого разбанить.</b>',
               'unbanned': '<b>{} Разбанен.</b>',
               'mute_none': '<b>Нет кого заткнуть.</b>',
               'muted': '<b>{} теперь молчит. </b>',
               'no_args': '<b>Аргументы некорректны.</b>',
               ...
               'del_u_kicking': '<b>Кик удаленных аккаунтов...\nOh~, Я могу это сделать?!</b>'}

    async def пикчаcmd(self, message):
        """Command .пикча changes the pic of the chat.\nUse: .ecp <reply to pic/sticker>."""
        if not message.chat:
    ...
    async def админcmd(self, message):
        """Command .админ for promote user to admin rights.\nUse: .promote <@ or reply> <rank>."""
        if not message.chat:
            return await utils.answer(message, self.strings('this_isn`t_a_chat', message))
        try:
    ...
    async def снятьcmd(self, message):
        """Command .снять for demote user to admin rights.\nUse: .demote <@ or reply>."""
    ...
    async def пинcmd(self, message):
        """Command .пин for pin message in the chat.\nUse: .pin <reply>."""
    ...
    async def анпинcmd(self, message):
        """Command .анпин for unpin message in the chat.\nUse: .unpin."""
        if not message.is_private:
    ...
    async def разбанcmd(self, message):
        """Command .разбан for unban the user.\nUse: .unban <@ or reply>."""
        if message.is_private:
            return await utils.answer(message, self.strings('this_isn`t_a_chat', message))
        try:
            reply = await message.get_reply_message()
    ...
    async def мутcmd(self, message):
        """Command .мут for mute the user.\nUse: .mute <@ or reply> <time (1м, 1ч, 1д)>."""
        if not message.is_private:
            args = utils.get_args_raw(message).split()
            reply = await message.get_reply_message()
            timee = False
    ...
    async def размутcmd(self, message):
        """Command .размут for unmute the user.\nUse: .unmute <@ or reply>."""
        if message.is_private:
            return await utils.answer(message, self.strings('this_isn`t_a_chat', message))
        try:
            reply = await message.get_reply_message()
    ...
    async def делюзcmd(self, message):
        """Command .делюз shows a list of all deleted accounts in the chat.\nUse: .удалить <clean>."""
        if message.is_private:
            return await utils.answer(message, self.strings('this_isn`t_a_chat', message))
        ...
        if con != "clean":
            ...
            del_status = f"<b>Найдены {del_u} удаленные аккаунты, кикните их испольуя </b><code>.delusers clean</code><b>.</b>"
        ...
        async for user in message.client.iter_participants(message.chat_id):
            if user.deleted:
                ...
                del_u += 1
        if del_u > 0:
            del_status = f"<b>Кикнуты {del_u} удалённые аккаунты.</b>"

        if del_a > 0:
            del_status = f"<b>Кикнуты {del_u} удаленнные аккаунты.\n" \
                         f"{del_a} но удалённые аккаунты админов не были удалены.</b>"
